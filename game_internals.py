...
def isGameOver(grid):
    for i in range(4):
        for j in range(4):
            if(grid[i][j] == 0): #means we have open space still
                return False
...
def add_number(board,number): #calling this function means that there is available space, make sure to check that before
# choosing a random index for
# row and column.
    r = random.randint(0, 3)
    c = random.randint(0, 3)
    while(board[r][c] != 0): #could pontentially run into infinite loop if we do not be careful
        r = random.randint(0, 3)
        c = random.randint(0, 3)    
